...
| Part | Preview |
|------|---------|
| Part Name | ![Part Name](parts/renders/part_name.png) |

> For flat models (no `parts/` subfolder), show a single preview in `renders/` next to the `.scad` file.
...
> - PNGs are generated by `scadm export-png` and stored in a `renders/` subfolder next to their source `.scad` file.
